...
## 实现思路
1.通过写一个余子式的函数，可以求出矩阵的行列式。通过余子式的定义，可以用递归的方式求出矩阵的行列式。
2.通过写一个矩阵的秩的函数，可以求出矩阵的秩。通过秩的定义，可以用上三角矩阵的方式求出矩阵的秩。
3.通过写一个矩阵的逆的函数，可以求出矩阵的逆。通过矩阵的乘法的定义，可以用行列式的求法求出矩阵的逆。
4.其他的都是常规做法

## 备选实现思路
1.在某些情况下，可以用高斯消元法求解线性方程组。
2.高斯消元法只用到一个n^2的计算，不用多算很多次余子式，直接得到变换过的矩阵，感觉更好做。

## 运行结果
![alt text](pic/微信图片_20240704130427.png)
备注其中rank_matrix一开始忘记填入错误输入了，已经改过了。